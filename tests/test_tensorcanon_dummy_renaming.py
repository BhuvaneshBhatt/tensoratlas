...
import sympy as sp
from tensoratlas import coordinate_chart, TensorObject, indexed, indices, normalize_indexed_expression, stronger_indexed_equal, indexed_equal, frame_basis, curvature_two_forms, first_structure_equation_residuals, young_projector_data, young_irreducible_canonicalize
from tensoratlas.tensor_algebra import metric_tensor, permutation_tensor

def test_young_and_symmetry_pipeline():
...
def test_cartan_identity_frame_pipeline():
    chart = coordinate_chart("Euclidean", "Cartesian", 3)
...
def test_epsilon_metric_pipeline_finishes():
    chart = coordinate_chart("Euclidean", "Cartesian", 3)
...
def test_symmetric_metric_swap():
    chart = coordinate_chart("Euclidean", "Cartesian", 3)
